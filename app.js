...
function watchFace(w, size = 'small') {
  ...
  if (size === 'big') {
    if (hasPhoto) {
      return `<div class="detail-watch-wrap">
        <img src="${w.image}?v=2" alt="${w.name}" class="detail-photo"
  ...
  if (hasPhoto) {
    return `<div class="card-face">
      <img src="${w.image}?v=2" alt="${w.name}" class="card-photo"
...
function renderCart() {
  ...
  body.innerHTML = cart.map(w => `
    <div class="cart-item">
      <div class="cart-face">
        ${w.image ? `<img src="${w.image}?v=2" style="width:100%;height:100%;object-fit:cover;border-radius:50%" onerror="this.style.display='none'">` : w.emoji}
      </div>
      <div class="cart-info">
